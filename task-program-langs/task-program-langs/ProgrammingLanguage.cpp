...
{
	(newYear >= 1943 && newYear <= 2021) ? this->year = newYear : this->year = 1943;
}
...
std::ostream& operator<<(std::ostream& out, const ProgrammingLanguage& current)
{
	out << current.name << std::endl
		...
		<< current.author << std::endl
		<< current.paradigm << std::endl
		<< current.helloWorld << std::endl
