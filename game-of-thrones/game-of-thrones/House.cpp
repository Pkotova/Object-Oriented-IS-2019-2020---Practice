...
void House::copy(const House& other)
{
	...
	setHouseWords(other.houseWords);
...
const char* House::getName()const
{
	return this->name;
}

const char* House::getLocation()const
{
	return this->location;
}

const char* House::getHouseWords()const
{
...
bool House::hasCharacter(const char* name)const
{
	for (int i = 0; i < this->size; i++)
	{
		if (strcmp(team[i]->getName(), name) == 0)
		{
			return true;
		}
	}
...
void House::srinkShow()const
{
	...
	std::cout << "Your chance to win the Game of Thrones is: : " << chance << "%" << std::endl;
	std::cout << std::endl;
}
